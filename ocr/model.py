...
from ocr import metrics
from ocr import sequence_layers
from ocr import utils

OutputEndpoints = collections.namedtuple(
    "OutputEndpoints",
    [
        "chars_logit",
        "chars_log_prob",
...
SequenceLogitsParams = collections.namedtuple(
...
EncodeCoordinatesParams = collections.namedtuple("EncodeCoordinatesParams", ["enabled"])
...
class Model(object):
    ...
    def default_mparams(self):
        return {
            ...
            "sequence_logit_fn": SequenceLogitsParams(
                use_attention=True,
            ...
            "sequence_loss_fn": SequenceLossParams(
                label_smoothing=0.1, ignore_nulls=True, average_across_timesteps=False
            ),
    ...
    """
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            predicted_chars, chars_log_prob, predicted_scores = self.char_predictions(
                chars_logit
            )
            if self._charset:
    ...
    """
        ...
        def assign_from_checkpoint(variables, checkpoint):
            logging.info(
                "Request to re-store %d weights from %s", len(variables), checkpoint
            )
            if not variables:
